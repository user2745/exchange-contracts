...
// produces names for pairs of tokens using AthleteX's naming scheme
library PairNamer {
    string private constant TOKEN_SYMBOL_PREFIX = 'AX:';
    ...
    // produces a pair symbol in the format of `AX:${symbol0}:${symbol1}${suffix}`
    function pairSymbol(
        address token0,
        address token1,
        string memory suffix
