...
contract AthleteXBridgeMigrationRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'AthleteXBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'AthleteXBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "AthleteXBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "AthleteXBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "AthleteXBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "AthleteXBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        IAthleteXERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = IAthleteXPair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = AthleteXLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = AthleteXLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IAthleteXPair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IAthleteXPair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(pairA != address(0), "AthleteXBridgeMigrationRouter: liquidityPairFrom address 0");
        require(pairA != address(0), "AthleteXBridgeMigrationRouter: liquidityPairTo address 0");
        require(pairA != pairB, "AthleteXBridgeMigrationRouter: Cant convert to the same liquidity pairs");
        require(
            IAthleteXPair(pairA).token0() == IAthleteXPair(pairB).token0() ||
            IAthleteXPair(pairA).token0() == IAthleteXPair(pairB).token1() ||
            IAthleteXPair(pairA).token1() == IAthleteXPair(pairB).token0() ||
            IAthleteXPair(pairA).token1() == IAthleteXPair(pairB).token1(),
            "AthleteXBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "AthleteXBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "AthleteXBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IAthleteXPair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IAthleteXPair(liquidityPairFrom).token0();
        if (
            IAthleteXPair(liquidityPairFrom).token0() == IAthleteXPair(liquidityPairTo).token0() ||
            IAthleteXPair(liquidityPairFrom).token0() == IAthleteXPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IAthleteXPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "AthleteXBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IAthleteXPair(liquidityPairTo).token0() ||
            newTokenAddress == IAthleteXPair(liquidityPairTo).token1(),
            "AthleteXBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != IAthleteXPair(liquidityPairFrom).token0()) {
                amountToSwap = amountTokenB;
            }
            _migrateToken(tokenToMigrate, amountToSwap);
        }
        if (IAthleteXPair(liquidityPairFrom).token0() != IAthleteXPair(liquidityPairTo).token0() &&
            IAthleteXPair(liquidityPairFrom).token1() != IAthleteXPair(liquidityPairTo).token1()
        ) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            IAthleteXPair(liquidityPairTo).token0(), IAthleteXPair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(IAthleteXPair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(IAthleteXPair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IAthleteXPair(pairAddress).getReserves();
        uint totalSupply = IAthleteXPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(liquidityPairFrom != address(0), "AthleteXBridgeMigrationRouter: liquidityPairFrom address 0 not supported");
        require(liquidityPairTo != address(0), "AthleteXBridgeMigrationRouter: liquidityPairTo address 0 not supported");
        ...
        (uint112 reserve0, uint112 reserve1,) = IAthleteXPair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = AthleteXLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
